...
def get_car_prices():
    """دریافت قیمت چهار خودروی منتخب از جدول ایران‌جیب."""
    from html.parser import HTMLParser
    class TableParser(HTMLParser):
        def __init__(self):
            super().__init__()
            self.in_tr = self.in_td = False
    ...
    r = requests.get(IRANJIB_CAR_URL, headers=headers, timeout=30)
    r.raise_for_status()
    parser = TableParser()
    parser.feed(r.text)
    ...
    targets = {
        "سورن TU5": ["سورن (TU5P)"],
        "۲۰۷ پانوراما": ["پژو 207 دنده‌ای پانوراما"],
        "دناپلاس MT6": ["دنا پلاس MT6"],
    ...
    for label, names in targets.items():
        ...
        for row in parser.rows:
            if len(row) >= 4 and any(name in row[0] for name in names):
                # ایران‌جیب: نام، بازار، کارخانه، تغییر
                match = row
                break
        if match:
            result.append({
            ...
            result.append({"name": label, "factory": "—", "market": "—", "change": "—"})
    return result


def build_car_table():
    """ساخت جدول کشویی قیمت چهار خودروی منتخب."""
    cars = get_car_prices()
    cells = [[
...
def send_telegram_rich(data):
    ...
    blocks = [
        ...
        {
            "type": "details",
            "summary": "🚗 قیمت خودروهای منتخب",
            "blocks": [build_car_table()],
        },
